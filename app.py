...
# Connect to the shared multi-device data engine singleton
...
def render_live_dashboard(main_view):
    ...
    # Advance shared telemetry data in server memory
    if streaming_active:
        shared_engine.tick_live_telemetry()

    current_df = shared_engine.df
    alerts_to_display = shared_engine.alerts_log

    if search_query.strip():
        q = search_query.lower()
        alerts_to_display = [
            a for a in shared_engine.alerts_log 
